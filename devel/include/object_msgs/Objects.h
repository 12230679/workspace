...
struct Objects_
{
  ...
  _header_type header;
   ...
   typedef std::vector< ::object_msgs::Object_<ContainerAllocator> , typename std::allocator_traits<ContainerAllocator>::template rebind_alloc< ::object_msgs::Object_<ContainerAllocator> >> _objects_vector_type;
  ...
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::object_msgs::Objects_<ContainerAllocator>& v)
  {
    ...
    for (size_t i = 0; i < v.objects_vector.size(); ++i)
    {
      if (false && i > 0)
        ...
        s << std::endl << indent << "  -";
      Printer< ::object_msgs::Object_<ContainerAllocator> >::stream(s, false ? std::string() : indent + "    ", v.objects_vector[i]);
    }
    if (v.objects_vector.empty() || false)
      s << "]";
    if (true || !indent.empty())
